...
//Envia el nombre de la session
routerLogIn.get('/get-session', async (req, res) => {
    if (req.session.nombre) {
        return res.send({nombre: req.session.nombre})
    }
    res.send({nombre:''})
})

//Inicia una nueva sesion con el nombre que proporciona el usuario
routerLogIn.post('/set-session', async (req, res) => {
    const body = req.body;
...
routerLogIn.get('/destroy', async (req, res) => {
    if (req.session.nombre) {
        req.session.destroy()
        res.send({ok:'sesion destruida'})
    }
})

//Extiende duracion de la session
routerLogIn.post('/reset-session', async (req, res) => {
    if (req.session.nombre){
        const body = req.body;
        req.session.nombre = body.nombre
        res.send({nombre: req.session.nombre})
    ...
    res.send({nombre: 'session expirada'})
})
